...
class CoreEngine:
    ...
    def _check_and_reload_config(self):
        """检查配置文件是否被修改，如果是则热重载（带校验，线程安全）"""
        current_mtime = self._get_config_mtime()
        if current_mtime != self._config_mtime:
            with self._config_lock:
                # 双重检查：获取锁后再次判断，避免重复重载
                if current_mtime == self._config_mtime:
                    return
                try:
    ...
    def _handle_agentic_flow(self, user_input: str, session_id: str, agent_result: dict, date: str) -> dict:
        ...
        # DAG经验总结：检查是否可沉淀为新技能（异步执行，不阻塞返回）
        try:
            if self.skill_manager and len([s for s in steps if s.get("type") == "dag_node_complete"]) >= 3:
                self.skill_manager.summarize_dag_experience(
                    user_input, steps,
                    execution_log=getattr(self.agentic_loop, '_dag_execution_log', None)
                )
        except Exception as e:
            self.logger.logger.error(f"DAG经验总结失败（不影响主流程）: {e}")
    ...
    def _save_agentic_as_dag(self, session_id: str, user_input: str, steps: list):
        ...
        try:
            ...
            logger.warning(f"DAG记录文件写入失败: {e}")
    ...
    def _incremental_save(self, session_id: str, user_input: str,
        ...
        try:
            ...
            logger.warning(f"增量保存失败（不影响主流程）: {e}")
