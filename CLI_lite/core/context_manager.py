...
class ConversationContext:
    ...
    max_tokens: int = 24000      # 最大token预算（60K上下文）
...
class ContextManager:
    ...
    def get_history_page(self, session_id: str, page: int = 1, per_page: int = 6) -> dict:
        """分页获取历史对话（per_page=6表示3轮，每轮含user+assistant两条消息）
        page=1 返回最新的per_page条消息，page=2 返回倒数per_page+1到2*per_page条，以此类推。
        返回: {"messages": [...], "has_more": bool, "total": int}
        """
        all_history = self._load_all_history(session_id)
        if not all_history:
        ...
        # page=1 -> 最新的per_page条（倒序分页，最新的先返回）
        end_idx = total - (page - 1) * per_page
        start_idx = max(0, end_idx - per_page)

        if end_idx <= 0:
            return {"messages": [], "has_more": False, "total": total}

        page_messages = all_history[start_idx:end_idx]
    ...
    def _compress_context(self, ctx: ConversationContext):
        ...
        # 压缩历史对话（保留最近3轮 = 6条消息，每次移除完整的一轮即2条）
        while ctx.total_tokens > ctx.max_tokens and len(ctx.history) > 6:
            ctx.history.pop(0)  # 移除最旧一轮的user消息
            if ctx.history:  # 安全检查：确保还有元素可弹出
                ctx.history.pop(0)  # 移除最旧一轮的assistant消息
            ctx.total_tokens = self._estimate_tokens(ctx)
