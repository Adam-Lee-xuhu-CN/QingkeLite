...
if (require.main === module) {
  ...
  for (var i = 0; i < args.length; i++) {
    if ((args[i] === '-i' || args[i] === '--input') && args[i + 1]) {
      ...
      outputFile = args[++i];
    } else if ((args[i] === '-t' || args[i] === '--theme') && args[i + 1]) {
      theme = args[++i];
    } else if (args[i] === '-l' || args[i] === '--list') {
      listThemes = true;
    } else if (args[i] === '-h' || args[i] === '--help') {
      console.log('PPT Maker - Markdown to PPTX Generator\n');
      console.log('Usage: node ppt-maker.js -i <input.md> -o <output.pptx> [-t theme]\n');
      console.log('Options:');
      console.log('  -i, --input   Input Markdown file path (required)');
      console.log('  -o, --output  Output PPTX file path (required, auto .pptx suffix)');
      console.log('  -t, --theme   Theme name (default: ocean)');
      console.log('  -l, --list    List all available themes');
  ...
  if (!inputFile || !outputFile) {
    console.error('Error: -i (input) and -o (output) are required. Use -h for help.');
    process.exit(1);
  }

  // 确保输出文件有.pptx后缀
  if (!outputFile.toLowerCase().endsWith('.pptx')) {
    outputFile += '.pptx';
  }

  try {
    var markdown = fs.readFileSync(inputFile, 'utf-8');
    var pres = createPPTX(markdown, { theme: theme });
    pres.writeFile({ fileName: outputFile }).then(function() {
      console.log('PPT generated: ' + path.resolve(outputFile));
    }).catch(function(err) {
