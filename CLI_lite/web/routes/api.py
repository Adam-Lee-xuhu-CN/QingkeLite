...
def _run_task_background(user_input, session_id, storage_entry, engine_ref, abort_set):
    ...
    try:
        ...
        if agent_result["action"] == "direct_reply":
            ...
            if use_agentic:
                final_response = ""
                last_node_result = ""  # 兜底：记录最后一个节点的结果
                steps = []
                ...
                # 兜底：如果 final_response 仍为空，使用最后一个节点的结果
                if not final_response and last_node_result:
                    final_response = last_node_result
                ...
                if session_id in abort_set:
                    ...
                    storage_entry['completed_at'] = time.time()
                    return
...
def create_api_blueprint(engine):
    ...
    _abort_sessions = set()
    # 轮询模式任务存储：{session_id: {'status': str, 'events': list, 'lock': Lock, 'started_at': float, 'completed_at': float}}
    _task_storage = {}
    _TASK_TTL = 1800  # 已完成任务保留30分钟后自动清理

    def _cleanup_expired_tasks():
        """清理已完成且超过TTL的任务"""
        now = time.time()
        expired = [
            sid for sid, task in _task_storage.items()
            if task['status'] in ('completed', 'error', 'aborted')
            and task.get('completed_at', 0) > 0
            and now - task['completed_at'] > _TASK_TTL
        ]
        for sid in expired:
            _task_storage.pop(sid, None)
        if expired:
            logging.debug(f"自动清理了 {len(expired)} 个过期任务")
    ...
    @api.route('/chat/status/<session_id>', methods=['GET'])
    def chat_status(session_id):
        """检查指定session是否有正在运行的任务（用于页面恢复）"""
        task = _task_storage.get(session_id)
        if not task:
        ...
        # 获取最新的DAG相关信息
        latest_dag_events = []
        for evt in reversed(task['events']):
            if evt.get('type') in ('dag_plan', 'dag_replan', 'dag_node_start', 'dag_node_complete', 'dag_planning'):
                latest_dag_events.append(evt)
                if len(latest_dag_events) >= 20:  # 最多返回20个最近的DAG事件
                    break
        latest_dag_events.reverse()
        
        return jsonify({
            "running": is_running,
            "status": task['status'],
            "event_count": event_count,
            "dag_events": latest_dag_events,
    ...
    def chat_history(session_id):
        """分页获取历史对话（倒序分页：page=1返回最新消息）
        query params: page (int, default 1), per_page (int, default 6, 即3轮对话)
        """
        ...
        try:
            result = engine.context_mgr.get_history_page(session_id, page, per_page)
            return jsonify(result)
        except Exception as e:
            logger.error(f"获取历史对话失败: {e}")
            return jsonify({"messages": [], "has_more": False, "total": 0})

    @api.route('/chat/clear-display', methods=['POST'])
    def chat_clear_display():
        """清空聊天栏显示（仅前端状态，不删除session文件和归档MD）"""
        return jsonify({"status": "ok", "message": "聊天栏已重置"})
    ...
    def system_restart():
        """请求重启青稞lite（前端会收到信号后执行页面重载）"""
        nonlocal _restart_requested
        _restart_requested = True
        logger.info("收到重启请求")
        return jsonify({"status": "restart_requested"})

    @api.route('/system/restart-status', methods=['GET'])
    def system_restart_status():
        """检查是否有重启请求（TUI轮询用）"""
        nonlocal _restart_requested
        requested = _restart_requested
        if requested:
            _restart_requested = False  # 消费信号
        return jsonify({"restart_requested": requested})
